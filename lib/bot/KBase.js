...
const KBase = {
  ...
  initSync: function() {
    ...
    const wikiDataDir = path.join(__dirname,
      '/../../data/fcc.wiki/deprecated wiki');
    ...
    fs.readdirSync(wikiDataDir).forEach(name => {
      if ((/md$/).test(name)) {
        ...
        const fileName = arr[arr.length - 1].toLowerCase();
  ...
  getTopicsAsList: function(keyword) {
    ...
    for (let i = 0; i < shortList.length; i++) {
      ...
      let link = Utils.linkify(topicData.dashedName, 'wiki',
